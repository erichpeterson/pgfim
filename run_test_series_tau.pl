...
my @TAU = ('0.5','0.6','0.7','0.8','0.9');
my $ETA = 0.5;
my $SUPPORT = 0.0;
my $str = "# " . " ETA_". $ETA . "DATA_" . $ARGV[0] . "\n";

my $exec_stats_fname = "ETA_" . $ETA . "DATA_" . $ARGV[0] . ".data";
...
foreach $tau_item (@TAU) {
	print TIMEFILE "$tau_item        ";
 
    foreach $tax (@input_tax_files) {
    	$SUPPORT = $dataset_sizes{$input_data_file} * $ETA;
    	#printf("$SUPPORT\n");
    	$itemset_output_file = "ETA_". $ETA . "TAU_" . $tau_item . "DATA_" . $input_data_file;
    	$itemset_output_file = $itemset_output_file . "TAX_" . $tax . ".out";
  		$cmd = "./PFGIM -inputDB $input_dir$input_data_file.dat -tau $tau_item -minsup $SUPPORT -print 1 ";
